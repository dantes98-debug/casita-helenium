...
export function PsychologistDashboard({ professional, appointments, patients, payments, roomBookings }: Props) {
  ...
  const commissionRate = professional.commission_rate ?? 0.3
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Hola, {professional.first_name}</h1>
        <p className="text-gray-500 text-sm">{professional.profession} &middot; {format(now, "EEEE d 'de' MMMM", { locale: es })}</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Card className="bg-blue-50 border-blue-100">
          <CardContent className="p-4">
            ...
            <p className="text-xs text-gray-400">{centerPatients.length} centro &middot; {ownPatients.length} propios</p>
          </CardContent>
        </Card>
        <Card className="bg-green-50 border-green-100">
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold text-green-700">${fmt(myNet)}</p>
        ...
        <Card className="bg-amber-50 border-amber-100">
          <CardContent className="p-4">
            ...
            <p className="text-xs text-gray-400">${fmt(roomCost)} este mes</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex items-center gap-2">
              <Clock className="h-4 w-4 text-blue-500" />Proximos turnos
            </CardTitle>
          </CardHeader>
          <CardContent>
            {upcomingAppts.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-4">Sin turnos proximos.</p>
            ) : (
              <div className="space-y-2">
                {upcomingAppts.map(a => (
                  <div key={a.id} className="flex items-center gap-3 p-2.5 rounded-lg border border-gray-100 hover:bg-gray-50">
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex items-center gap-2">
              <DollarSign className="h-4 w-4 text-green-500" />Liquidacion del mes
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between text-gray-400 text-xs pl-4">
                <span>Comision centro ({Math.round(commissionRate * 100)}%)</span>
